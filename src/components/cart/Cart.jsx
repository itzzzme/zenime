...
function Cart({ label, data, path }) {
    ...
    return (
        <div className="flex flex-col w-1/4 space-y-7 max-[1200px]:w-full">
            <h1 className='font-bold text-2xl text-[#ffbade] max-md:text-xl'>{label}</h1>
            <div className='w-full space-y-4 flex flex-col'>
                {data && data.slice(0, 5).map((item, index) => (
                    <div key={index} style={{ borderBottom: "1px solid rgba(255, 255, 255, .075)" }} className="flex pb-4 items-center relative"
            ...
            ${tooltipHorizontalPosition === "left-1/2" ? "translate-x-[-100px]" : "translate-x-[-200px]"} 
            ...
            ${hoveredItem === item.id ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}
                        ...
                        <div className='flex flex-col ml-4 space-y-2 w-full'>
                            <Link to={`/${item.id}`} className='w-full line-clamp-2 text-[1em] font-[500] hover:cursor-pointer hover:text-[#ffbade] transform transition-all ease-out max-[1200px]:text-[14px]'>
                            ...
                            <div className='flex items-center flex-wrap w-fit space-x-1'>
                                ...
                                <div className='flex items-center w-fit pl-1 gap-x-1'>
                                    ...
                                    <p className='text-[14px] text-[#D2D2D3]'>{item.tvInfo.showType}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ))}
                <Link to={`/${path}`} className='flex w-fit items-baseline rounded-3xl gap-x-2 group'>
                    <p className='text-white text-[17px] h-fit leading-4 group-hover:text-[#ffbade] transform transition-all ease-out'>View more</p>
