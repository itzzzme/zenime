  ...
  useEffect(() => {
    const loadIframeUrl = async () => {
      ...
      if (lowerName === "hd-1" || lowerName === "hd-4") {
        ...
        setIframeSrc(`${baseURL}/${animeInfo?.anilistId}/${episodeNum}/${servertype}`)
      } else if(lowerName === "vidstreaming" || lowerName === "vidcloud" || lowerName === "douvideo"){
        const {data}=await axios.get(`${API_URL}/stream?id=${animeId}?ep=${episodeId}&server=${lowerName}&type=${servertype}`)
