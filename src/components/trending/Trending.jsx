...
const Trending = ({ trending }) => {
  ...
  return (
    <div className='mt-6 max-[1200px]:px-4 max-md:px-0'>
      <h1 className='text-[#ffbade] text-2xl font-bold max-md:pl-4'>Trending</h1>
        ...
        >
          {trending &&
            trending.map((item, idx) => (
              <SwiperSlide key={idx} className="text-center flex text-[18px] justify-center items-center">
                <div className="w-full h-auto pb-[115%] relative inline-block overflow-hidden max-[575px]:pb-[150%]">
                  <div className="absolute left-0 top-0 bottom-0 overflow-hidden w-[40px] text-center font-semibold bg-[#201F31] max-[575px]:top-0 max-[575px]:h-[30px] max-[575px]:z-[9] max-[575px]:bg-white">
                    <span className="absolute left-0 right-0 bottom-0 text-[24px] leading-[1.1em] text-cente z-[9] transform -rotate-90 max-[575px]:transform max-[575px]:rotate-0 max-[575px]:text-[#111] max-[575px]:text-[18px] max-[575px]:leading-[30px]">
                      {item.number}
                    </span>
                    <div className="w-[150px] h-fit text-left transform -rotate-90 absolute bottom-[100px] left-[-55px] leading-[40px] text-ellipsis whitespace-nowrap overflow-hidden text-white text-[16px] font-medium">{language === 'EN' ? item.title : item.japanese_title}</div>
                  </div>
                  <Link to={`/${item.id}`} className="inline-block bg-[#2a2c31] absolute w-auto left-[40px] right-0 top-0 bottom-0 max-[575px]:left-0 max-[575px]:top-0 max-[575px]:bottom-0">
                    <img src={item.poster} alt={item.title} className="block w-full h-full object-cover hover:cursor-pointer" title={item.title} loading="lazy" />
                  </Link>
                </div>
              </SwiperSlide>
            ))}
