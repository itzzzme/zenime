...
export const useWatch = (animeId, initialEpisodeId) => {
    ...
    const fetchServers = async () => {
      ...
      try {
        ...
        const initialServer =
          ...
          filteredServers.find(s => s.type === savedServerType && ["HD-1", "HD-2", "HD-3", "HD-4", "HD-5","Vidstreaming","Vidcloud","DouVideo"].includes(s.serverName)) ||
          filteredServers[0];
  ...
  useEffect(() => {
    if (
      ...
      return;
    const iframeServers = ["hd-1", "hd-4", "hd-5", "vidstreaming", "vidcloud", "douvideo"];
